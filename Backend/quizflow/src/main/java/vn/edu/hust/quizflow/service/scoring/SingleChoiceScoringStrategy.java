...
/**
 * Chiến lược chấm điểm dành riêng cho loại câu hỏi TRẮC NGHIỆM 1 LỰA CHỌN (Single Choice).
...
public class SingleChoiceScoringStrategy implements ScoringStrategy {
     ...
     * Thuật toán này đơn giản nhất: So sánh chuỗi (String) khớp 100% thì ăn trọn điểm.
     */
    @Override
    ...
    public BigDecimal calculateScore(Object studentAnswer, Map<String, Object> questionMetadata, BigDecimal maxScore) {
        ...
        // Lấy danh sách đáp án đúng từ DB. Dù là SINGLE nhưng cấu trúc DB vẫn lưu dưới dạng List để dùng chung.
        List<String> correctAnswers = (List<String>) questionMetadata.get("correctAnswers");
        if (correctAnswers == null || correctAnswers.isEmpty()) {
            return BigDecimal.ZERO;
        }

        // Vì là trắc nghiệm 1 lựa chọn, nên đáp án đúng duy nhất chắc chắn nằm ở phần tử đầu tiên (index 0)
        ...
        // So sánh chính xác tuyệt đối (không phân biệt chữ thường/hoa như câu Điền từ, vì ID đáp án là cố định)
